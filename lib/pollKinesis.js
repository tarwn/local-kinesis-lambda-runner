...
module.exports = (kinesis, StreamName, logger) => {
  ...
  const callback = (err, result) => err ? logger.error(err) : logger.log(`Handler succeeded: ${result}`);
  ...
  const pollKinesis = lambda => firstShardIterator => {
    const fetchAndProcessRecords = shardIterator => kinesis.getRecords({ ShardIterator: shardIterator }).promise().then(records => records.Records.reduce(reduceRecord(lambda), Promise.resolve()).then(wait(500)).then(() => fetchAndProcessRecords(records.NextShardIterator)));
    return fetchAndProcessRecords(firstShardIterator);
  };

  const run = lambda => {
    const loop = () => kinesis.describeStream({ StreamName }).promise().then(stream => {
    ...
    }).then(pollKinesis(lambda)).catch(err => {
      logger.error(err);
